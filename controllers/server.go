...
func GetContainerCpu(ctx *gin.Context)  {
	...
	//	returnMsg(ctx, configs.ERROR_PARAMS, "", "container_ids can not null")
	//	return
	//}
	...
	SQL := fmt.Sprintf(baseSql, filter.Field, filter.Field, filter.DashboardTime, filter.GetInterval())
...
func GetContainerMax(ctx *gin.Context)  {
	...
	SQL := fmt.Sprintf(baseSql, filter.Field, filter.Field, filter.DashboardTime, filter.HostName, filter.GetInterval())
	res,err := services.Service.Query(SQL)
	if err != nil{
		returnMsg(ctx, configs.ERROR_DATABASE, nil, err.Error())
		return
...
func GetSystemCpu(ctx *gin.Context)  {
	...
	SQL := fmt.Sprintf(baseSql, filter.Field, filter.DashboardTime, filter.HostName, filter.GetInterval())
	res,err := services.Service.Query(SQL)
	if err != nil{
		returnMsg(ctx, configs.ERROR_DATABASE, nil, err.Error())
		return
